...
使用 `import { ... } from "@components/mdx"` 輕鬆導入自訂組件!
...
<Accordion title="Money 是一隻鯊魚">Yeahhhhh</Accordion>;
```
...
您可以輕鬆地使用 `className` 來達到這個效果，使用 `###` (h3) 標題新增一個步驟：
...
<FileTree>
  <FileTree.Folder name="你的檔案" defaultOpen>
    <FileTree.Folder name="frameworks" defaultOpen>
      <FileTree.File name="react.mdx" />
      <FileTree.File name="svelte.mdx" />
      <FileTree.File name="vue.mdx" />
