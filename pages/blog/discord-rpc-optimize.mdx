...
title: 2022 年如何自訂 Discord 遊戲狀態（Rich Presence）
authors: [kay]
description: "Discord 遊戲狀態是一個可以顯示你目前狀態更多資訊的東西，可以看到朋友都能顯示他在玩什麼遊戲、正在用什麼程式語言和 IDE 寫程式。此文章會教大家使用 Python、JavaScript、TypeScript 和 C# 來設定 Discord 豐富狀態的多種方法！"
tags:
  [
	"Discord",
	"dc",
...
![Discord 自訂遊戲狀態](@static/blog/discord-rpc-customize/discord-rpc-customize.jpeg)
...
Visual Studio Code 是一款微軟開發的一款非常好用的一款編輯器，除了 Git、除錯功能外，還有非常大的主題與延伸模組庫，連 Discord Rich Presence 都有非常多的延伸模組！以下就為各位介紹如何安裝與設定：

首先，請先前往延伸模組商店，並在搜尋中輸入「Discord Rich Presence」，它會列出相關延伸模組，找到自己喜歡的模組後，請按下安裝按鈕進行安裝（在此我安裝這張圖中的第 2 個）
...
再來，寫一個 `ready` 事件監聽器，當 RPC 用戶端成功上線後，將您的 Rich Presence 修改成您指定的資訊，
